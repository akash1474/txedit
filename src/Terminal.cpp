...
void Terminal::Render() {
    ImGui::Begin("Terminal");
        ...
        if (ImGui::IsWindowHovered()) this->HandleInputs();
...
void Terminal::ShellReader() {
    ...
    while (!mTerminalThread) {
        ...
        if(!buffer.empty()){
            ...
            if(!mBuffer.empty() && mBuffer.back()=='\x07') mBuffer.pop_back();
...
void Terminal::RunCommand(std::string& command) {
    if (mIsCommandRunning || command=="exit") return;
    if(command.empty()){
        if(mLines.back().back()=='>') mLines.push_back(mLines.back());
    ...
    if(command=="cls"){
        ...
        std::string dir=line.substr(0,line.find_first_of('>')+1);
        mBuffer.clear();
        mLines.clear();
...
void Terminal::AppendText(const std::string& text)
{
    ...
    if(!isMultiLineText){
        if(mLines.empty()) mLines.push_back(segment);
        else mLines[aWhere.mLine].insert(idx,text);
    ...
    if(mLines.empty()) mLines.push_back(segment);
    else mLines[aWhere.mLine]+=segment;
...
void Terminal::Draw(){
    ...
    if (mPosition.x != mWindow->Pos.x || mPosition.y != mWindow->Pos.y) mRecalculateBounds = true;
    if (mRecalculateBounds) UpdateBounds();
    ...
    if (!ImGui::ItemAdd(mBounds, id)) return;
...
void Terminal::HandleInputs(){
    ...
    //Mouse Click And Dragging
        ...
        if((ImGui::GetMousePos().y-mPosition.y) < mTitleBarHeight) return;
    ...
    //Left
        ...
        if(mSelectionMode!=SelectionMode::Normal) ExitSelectionMode();
        if(cursorPosition.mColumn>0 && cursorPosition.mColumn<=max) cursorPosition.mColumn--;
    }
    //Right
    ...
    else if (!ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_Backspace)))
    {
        if(cursorPosition.mColumn>mReadOnlyCoords.mColumn && cursorPosition.mColumn<=max)
        {
            ...
            while(cidx>0 && IsUTFSequence(line[cidx])) 
                cidx--;
    ...
    //Paste
        ...
        if(!text.empty()) AppendText(text);
    }

    //Character Input
    if (!io.InputQueueCharacters.empty()) {

        if (mSelectionMode == SelectionMode::Word) mSelectionMode=SelectionMode::Normal;
...
void Terminal::PushHistory(const std::string& cmd)
{
...
{
    if(keyUp && mHistoryIdx>0) 
        mHistoryIdx--;
    else if(!keyUp && mHistoryIdx<mHistory.size()-1) 
        mHistoryIdx++;

    mLines[mReadOnlyCoords.mLine].erase(mLines[mReadOnlyCoords.mLine].find_first_of('>')+1);
    mLines[mReadOnlyCoords.mLine]+=mHistory[mHistoryIdx];
    mState.mCursorPosition=GetCommandInsertPosition();
}
