...
void Application::CenterWindowOnScreen(){
	...
	// Get().GetCoreState()->position=ImVec2((horizontal - Get().GetCoreState()->width) * 0.5f, (vertical - Get().GetCoreState()->height) * 0.5f);
	// glfwSetWindowPos(Get().mWindow,(int)Get().GetCoreState()->position.x,(int)Get().GetCoreState()->position.y);
}
...
void Application::HandleArguments(std::wstring commands){
	...
	if(!wArgList){GL_ERROR("Failed to parse args"); return; }
